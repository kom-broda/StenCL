...
void cl_launch_kernel() {
	int i;
	/*const size_t globalWorkSize = NX * NY * NZ;
	const size_t localWorkSize = DIM_X * DIM_Y * DIM_Z;*/
	...
	for(i = 0; i < TMAX; i++){
		...
		*/
		...
		errcode |= clSetKernelArg(clKernel1, 2, DIM_X*DIM_Y*DIM_Z*sizeof(float), NULL);
